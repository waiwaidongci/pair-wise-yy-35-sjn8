...
READING_ROLES=set(['dosimetrist', 'radiation_officer']); READING_CONFIRM_ROLES=set(['radiation_officer']); READING_CORRECT_ROLES=set(['radiation_officer', 'health_physicist'])
READING_STATUSES=['pending', 'confirmed']; DEFAULT_DOSE_LIMIT=50.0
...
def net_dose(raw_dose,background):
    return max(0.0,round(float(raw_dose)-float(background),6))
def follow_up_required(dose_current,dose_limit):
    return dose_limit is not None and dose_current>=dose_limit
def remaining_report_hours(deadline_hours,created_at,now=None):
    now=now or datetime.now(timezone.utc)
    created=datetime.fromisoformat(created_at)
    if created.tzinfo is None: created=created.replace(tzinfo=timezone.utc)
    elapsed=(now-created).total_seconds()/3600.0
    return max(0.0,round(float(deadline_hours)-elapsed,2))
